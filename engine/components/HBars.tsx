...
export const HBars: React.FC<{x: number; y: number; width: number; bars: Bar[]; max: number; min?: number; rowH?: number; barH?: number;
  ...
  return (
    <div style={{position: 'absolute', left: x, top: y}}>
      {title && <div style={{fontFamily: font.serif, fontSize: 46, fontWeight: 600, color: color.ink, marginBottom: 34, opacity: ramp(t, titleAt, 0.5)}}>{title}</div>}
          ...
          return (
            <div key={i} style={{position: 'relative', height: rowH, opacity: ramp(t, b.at - 0.1, 0.4)}}>
              <div style={{position: 'absolute', left: 0, width: labelW - 24, top: (rowH - barH) / 2, height: barH, display: 'flex', alignItems: 'center',
                justifyContent: 'flex-end', fontFamily: font.sans, fontSize: 30, fontWeight: 500, color: color.ink, textAlign: 'right'}}>{b.label}</div>
              ...
              <div style={{position: 'absolute', left: labelW + sx(b.value) * p + 18, top: (rowH - barH) / 2, height: barH, display: 'flex', alignItems: 'center',
                fontFamily: font.sans, fontSize: 32, fontWeight: 600, color: b.col ?? color.indigo, whiteSpace: 'nowrap', ...num}}>
                {b.valueText ?? fmt(b.value * p) + suffix}
                {b.note && <span style={{fontWeight: 400, color: color.inkSoft, marginLeft: 16, fontSize: 26, opacity: b.noteAt === undefined ? 1 : ramp(t, b.noteAt, 0.5)}}>{b.note}</span>}
              </div>
            </div>
          );
        })}
